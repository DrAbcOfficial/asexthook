...
void RegisterAngelScriptHooks(){
	...
	g_AngelHook.pBreakableDie = ASEXT_RegisterHook("Post call after a breakable died", StopMode_CALL_ALL, 2, ASFlag_MapScript | ASFlag_Plugin, "Entiry", "BreakableDie", "CBaseEntiry@ pBreakable");
	g_AngelHook.pBreakableTakeDamage = ASEXT_RegisterHook("Post call after a breakable took damage", StopMode_CALL_ALL, 2, ASFlag_MapScript | ASFlag_Plugin, "Entiry", "BreakableTakeDamage", "CBaseEntiry@ pBreakable, entvars_t@ pevInflictor, float flDamage, int bitDamageType");
}